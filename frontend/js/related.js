...
const OURURL = `${USABLE_URL}/replay/findOne/`
...
genButton.addEventListener('click', () => {
	...
	fetch(`http://165.22.130.92/replay/${genInput.value}`, {
		...
		.then(resp => {
			...
			let textGen = `<p>A total of ${resp.tests.length} tests have been generated. Click an ID to generate a test review:</p>
			<ul>
			`;
			for (let i = 0; i < resp.tests.length; i++) {
				textGen += `<li><a onClick="genText('${resp.tests[i]}')">${resp.tests[i]}</a></li>`
			}
			textGen += '</ul>';
			listInfo.innerHTML = textGen;
		})
})
...
function copyText() {
	document.getElementById('usableConsoleInfo').select();
